...
--------- Procedure to loading data in the SRC_GLOBAL_SALES_ARCH ---------

CREATE OR REPLACE PROCEDURE "BL_CL"."LOAD_DATA_TO_SRC_GLOBAL_SALES_ARCH"()
LANGUAGE plpgsql
AS
$$
DECLARE
...
BEGIN
    ...
    BEGIN
	    INSERT INTO "SA_GLOBAL_SERVER_SALES"."SRC_GLOBAL_SALES_ARCH"
		SELECT *, 
    ...
    RAISE NOTICE 'Numbers of inserted/updated rows to SRC_GLOBAL_SALES_ARCH: %/%; Is success: %', 
                 v_inserted_rows, v_updated_rows, v_success_flag;
   
	CALL "BL_CL"."LOG_PROCEDURE_EXECUTION"(
    	'LOAD_DATA_TO_SRC_GLOBAL_SALES_ARCH',
	    'SA_GLOBAL_SERVER_SALES.SRC_GLOBAL_SALES_ARCH',
	    v_inserted_rows,
	    v_updated_rows,
	    v_success_flag,
