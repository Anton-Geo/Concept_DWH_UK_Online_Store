...
CREATE OR REPLACE PROCEDURE "BL_CL"."LOAD_DATA_FROM_UE_TO_WRK_PRODUCTS_SCD"()
LANGUAGE plpgsql
AS
$$
DECLARE
...
BEGIN
    ...
    BEGIN
		...
		USING (
		    SELECT DISTINCT
		           ...
		           FIRST_VALUE(CASE WHEN UnitCost ~ '^[0-9.]*$' 
		                             AND LENGTH(REPLACE(UnitCost, '.', '')) <= 10 THEN UnitCost 
	...
	CALL "BL_CL"."LOG_PROCEDURE_EXECUTION"(
    	'LOAD_DATA_FROM_UE_TO_WRK_PRODUCTS_SCD',
